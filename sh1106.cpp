...
#define SH1106_I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED \
...
#define SH1106_SET_SEGMENT_REMAP_FLIP	0xA0 // 0 to 127
...
const uint8_t init_array[] = { SH1106_DISPLAYOFF, // 0xAE
...
void sh1106::refresh( void )
{
	for ( uint16_t page = 0; page < SH1106_MAX_PAGE_COUNT; page++ )
	{
		...
		if ( !( ( ( pageUpdateMask ) >> ( page ) ) & 0x01 ) ) continue;
...
/*
 * Send the frame buffer without optimizations
 */
void sh1106::refresh_force( void )
{
	pageUpdateMask = 0xff;
	refresh();
}
...
void sh1106::drawPixel( uint8_t x, uint8_t y, uint8_t color )
{
	...
	/* clip */
	if ( x >= SH1106_W ) return;
	if ( y >= SH1106_H ) return;
 ...
 */
void sh1106::drawImage(
	uint8_t x, uint8_t y, const unsigned char *input, uint8_t width, uint8_t height, uint8_t color_mode )
{
	uint8_t x_absolute;
	uint8_t y_absolute;
	uint8_t pixel;
	...
	for ( uint8_t line = 0; line < height; line++ )
	{
		...
		// SH1106 is in vertical mode, yet we want to draw horizontally, which necessitates assembling the output bytes
		// from the input data bitmask for current pixel in vertical (output) byte
...
void sh1106::drawstr_sz( uint8_t x, uint8_t y, char *str, uint8_t color, font_size_t font_size )
{
	...
	while ( ( c = *str++ ) )
	{
		...
		if ( x > 128 - FONT_WIDTH * font_size ) break;
	}
}
...
uint8_t sh1106::i2c_send( uint8_t addr, uint8_t *data, uint8_t sz )
{
	...
	while ( ( I2C1->STAR2 & I2C_STAR2_BUSY ) && ( timeout-- ) )
		;
	if ( timeout == -1 ) return i2c_error( 0 );
	...
	while ( ( !i2c_chk_evt( SH1106_I2C_EVENT_MASTER_MODE_SELECT ) ) && ( timeout-- ) )
		;
	if ( timeout == -1 ) return i2c_error( 1 );
	...
	while ( ( !i2c_chk_evt( SH1106_I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED ) ) && ( timeout-- ) )
		;
	if ( timeout == -1 ) return i2c_error( 2 );
	...
	while ( sz-- )
	{
		...
		while ( !( I2C1->STAR1 & I2C_STAR1_TXE ) && ( timeout-- ) )
			;
		if ( timeout == -1 ) return i2c_error( 3 );
	...
	while ( ( !i2c_chk_evt( SH1106_I2C_EVENT_MASTER_BYTE_TRANSMITTED ) ) && ( timeout-- ) )
		;
	if ( timeout == -1 ) return i2c_error( 4 );
